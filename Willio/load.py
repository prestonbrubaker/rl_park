...
truncate_frames = 100  # Number of frames to truncate from the end
...
def compute_q_values(dataset, gamma, truncate_frames):
    ...
    for i in range(len(dataset) - truncate_frames):
        ...
        dS = item['score'].item() - (dataset[i-1]['score'].item() if i > 0 else 0)  # Score differential
        ...
        # Q-value for each action (simplified: assume action taken gives dS + future reward)
        q_values = torch.zeros(4, dtype=torch.float32)
        for action_idx in range(4):
            if control[action_idx] == 1:
                q_values[action_idx] = dS + gamma * future_reward
            else:
                q_values[action_idx] = 0  # No reward if action not taken (can be adjusted)
